...
dirmain = strcat('/home/varsha/KTH courses/Pattern Recognition/Project/patternRecognition/PattRecClasses/TrainingDataUsed/samples/*0',num2str(classTest),'.wav');

listing = dir(fullfile(dirmain));
...
% end
%%
TrainingSamples = floor((5/6) * rows);
TestingSamples = rows - TrainingSamples;
row_i = 0;
mfccs = runGetFeatures(listing(1).name);
xT = mfccs;
lData = size(mfccs,2);
for i =2:TrainingSamples
